...
package com.bytedesk.im.ui.recorder;
...
//        mLowStorageNotification = null;
        mRemainingTimeCalculator = new KFRemainingTimeCalculator();
...
//        mNotifiManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        ...
        mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "bytedesk:SoundRecorder");
    ...
    private void localStartRecording(int outputfileformat, String path, boolean highQuality,
        ...
        if (mRecorder == null) {
            ...
            try {
                ...
                sendErrorBroadcast(KFRecorder.INTERNAL_ERROR);
                mRecorder.reset();
                mRecorder.release();
                mRecorder = null;
                return;
            ...
            try {
                ...
                if (isInCall) {
                    sendErrorBroadcast(KFRecorder.IN_CALL_RECORD_ERROR);
                } else {
                    sendErrorBroadcast(KFRecorder.INTERNAL_ERROR);
                }
                mRecorder.reset();
...
//        		new Notification(
//        				KFResUtil.getResofR(this).getDrawable("appkefu_record_notification"),
//                getString(KFResUtil.getResofR(this).getString("appkefu_notification_recording")), System.currentTimeMillis());
...
//        		getString(KFResUtil.getResofR(this).getString("app_name")),
//                getString(KFResUtil.getResofR(this).getString("appkefu_notification_recording")), pendingIntent);
//
...
//            mLowStorageNotification =
//            		new Notification(
//            				KFResUtil.getResofR(this).getDrawable("appkefu_record_notification_full"),
//                    getString(KFResUtil.getResofR(this).getString("appkefu_notification_recording")), System.currentTimeMillis());
...
//        pendingIntent = PendingIntent
//                .getActivity(this, 0, new Intent(this, KFChatActivity.class), 0);
//
//        mLowStorageNotification.setLatestEventInfo(this, getString(KFResUtil.getResofR(this).getString("app_name")),
//                getString(KFResUtil.getResofR(this).getString("appkefu_notification_record_warning")), pendingIntent);
...
//        mLowStorageNotification = null;
//
//        Notification notification = new Notification(KFResUtil.getResofR(this).getDrawable("appkefu_record_notification"),
//                getString(KFResUtil.getResofR(this).getString("appkefu_notification_record_stopped")), System.currentTimeMillis());
...
//
//        notification.setLatestEventInfo(this, getString(KFResUtil.getResofR(this).getString("app_name")),
//                getString(KFResUtil.getResofR(this).getString("appkefu_notification_record_stopped")), pendingIntent);
    ...
    private void updateRemainingTime() {
        ...
        if (t <= 0) {
            ...
            return;
                ...
                && mRemainingTimeCalculator.currentLowerLimit() != KFRemainingTimeCalculator.FILE_SIZE_LIMIT) {
...
//    }
    ...
    public static void startRecording(Context context, int outputfileformat, String path,
        ...
        Intent intent = new Intent(context, KFRecorderService.class);
...
//    }
	...
	{		
		...
		if (mRecorder != null) {
			try {
				amplitude = (mRecorder.getMaxAmplitude() / 2700.0);
			}
