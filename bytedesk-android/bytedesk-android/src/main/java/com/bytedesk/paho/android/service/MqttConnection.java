...
import com.bytedesk.paho.android.service.MessageStore.StoredMessage;
...
/**
 * <p>
 * MqttConnection holds a MqttAsyncClient {SHORT_LINK_HOST,port,clientId} instance to perform
...
class MqttConnection implements MqttCallbackExtended {
	...
	//store login ActivityToken for reconnect
	private String reconnectActivityToken = null;

	// our client object - instantiated on login
	 ...
	 * @param serverURI
	 *            the URI of the MQTT server to which we will login
	 * @param clientId
	...
	MqttConnection(MqttService service, String serverURI, String clientId,
		...
		stringBuilder.append(" ");
		stringBuilder.append("on SHORT_LINK_HOST ");
		...
		try {
			...
			IMqttActionListener listener = new MqttConnectionListener(
				...
				public void onSuccess(IMqttToken asyncActionToken) {
					...
					service.traceDebug(TAG, "login success!");
				}
				...
				public void onFailure(IMqttToken asyncActionToken,
					...
					service.traceError(TAG,
							"login fail, call login to reconnect.reason:"
			...
			if (myClient != null) {
				if (isConnecting ) {
					...
					service.traceDebug(TAG,"Do Real login!");
			...
			service.traceError(TAG, "Exception occurred attempting to login: " + e.getMessage());
	...
	* Declare as synchronized to avoid multiple calls to this method to send login
	* multiple times 
	*/
	synchronized void reconnect() {
		...
		if(connectOptions.isAutomaticReconnect()){
			...
			// use the activityToke the same with action login
