...
import com.bytedesk.core.event.ThreadEntityEvent;
...
import com.bytedesk.core.event.WebRTCEvent;
...
//            String encryptString = AESCipher.aesEncryptString(content);
    ...
    private void onMessagesArrived(String topic, MqttMessage message) {
        ...
        try {
            ...
            threadEntity.setTid(messageProto.getThread().getTid());
            ...
            if (type.equals(BDCoreConstant.MESSAGE_TYPE_TEXT)) {
                ...
                sendReceipt = true;
                messageEntity.setImageUrl(messageProto.getImage().getImageUrl());
                //
                // 广播收到的消息
                EventBus.getDefault().post(new MessageEntityEvent(messageEntity));
                EventBus.getDefault().post(new ThreadEntityEvent(threadEntity));
                ...
                sendReceipt = true;
                messageEntity.setVoiceUrl(messageProto.getVoice().getVoiceUrl());
                //
                // 广播收到的消息
                EventBus.getDefault().post(new MessageEntityEvent(messageEntity));
                EventBus.getDefault().post(new ThreadEntityEvent(threadEntity));
                ...
                sendReceipt = true;
                messageEntity.setFileUrl(messageProto.getFile().getFileUrl());
                //
                // 广播收到的消息
                EventBus.getDefault().post(new MessageEntityEvent(messageEntity));
                EventBus.getDefault().post(new ThreadEntityEvent(threadEntity));
                ...
                sendReceipt = true;
                messageEntity.setVideoOrShortUrl(messageProto.getVideo().getVideoOrShortUrl());
                //
                // 广播收到的消息
                EventBus.getDefault().post(new MessageEntityEvent(messageEntity));
                EventBus.getDefault().post(new ThreadEntityEvent(threadEntity));
            ...
            if (sendReceipt && !isSelfSend) {
                ...
                sendMessageProtobuf(BDCoreUtils.uuid(), BDCoreConstant.MESSAGE_TYPE_NOTIFICATION_RECEIPT, "content",
                        threadEntity.getTid(), threadEntity.getTopic(), threadEntity.getType(), threadEntity.getNickname(), threadEntity.getAvatar(), threadEntity.getClient(),
                        extraParam);
            }
            //
