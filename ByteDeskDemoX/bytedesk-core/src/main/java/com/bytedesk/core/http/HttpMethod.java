...
//            jsonObject.put("country", country);
     ...
     * @param baseCallback
     */
    public static void markClearThreadMessage(final Context context, final String tid, final BaseCallback baseCallback) {
        //
        String client = BDCoreConstant.CLIENT_ANDROID;
        //
        JSONObject jsonObject = new JSONObject();
        ...
        HttpManager.getInstance(context).getWebService()
            .markClearThreadMessage(requestBody)
            .enqueue(new Callback<ResponseBody>() {
                    ...
                    try {
                        ...
                        if (response.code() == 401) {
                            ...
                            if (null != bodyJson) {
                                ...
                                if (status_code == 200) {
                                    // 更新本地数据
                                    BDRepository.getInstance(context).deleteThreadMessage(tid);
                                }
                                //
                                baseCallback.onSuccess(jsonObject);
                            }
...
//                        resultCallback(response, baseCallback);
     ...
     * @param baseCallback
     */
    public static void markClearContactMessage(final Context context, final String uid, final BaseCallback baseCallback) {
        //
        String client = BDCoreConstant.CLIENT_ANDROID;
        //
        JSONObject jsonObject = new JSONObject();
        ...
        HttpManager.getInstance(context).getWebService()
                .markClearContactMessage(requestBody)
                .enqueue(new Callback<ResponseBody>() {
                        ...
                        try {
                            ...
                            if (response.code() == 401) {
                                ...
                                if (null != bodyJson) {
                                    ...
                                    if (status_code == 200) {
                                        // 更新本地数据
                                        BDRepository.getInstance(context).deleteContactMessage(uid);
                                    }
                                    //
                                    baseCallback.onSuccess(jsonObject);
                                }
...
//                        resultCallback(response, baseCallback);
     ...
     * @param baseCallback
     */
    public static void markClearGroupMessage(final Context context, final String gid, final BaseCallback baseCallback) {
        //
        String client = BDCoreConstant.CLIENT_ANDROID;
        //
        JSONObject jsonObject = new JSONObject();
        ...
        HttpManager.getInstance(context).getWebService()
            .markClearGroupMessage(requestBody)
            .enqueue(new Callback<ResponseBody>() {
                @Override
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    //
                    try {
                        ...
                        if (response.code() == 401) {
                            ...
                            if (null != bodyJson) {
                                ...
                                if (status_code == 200) {
                                    // 更新本地数据
                                    BDRepository.getInstance(context).deleteGroupMessage(gid);
                                }
                                //
                                baseCallback.onSuccess(jsonObject);
                            }
